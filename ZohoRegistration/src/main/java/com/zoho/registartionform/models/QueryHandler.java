...
public class QueryHandler implements Persistance{
	...
	public ArrayList<AccountInfo>  wholeAccount() {
		ResultSet account_rs = null;
		Statement stmt=null;
		ArrayList<AccountInfo> accountList = new ArrayList();
		try {
			stmt = connection.createStatement();
			sql = "select * from account_details where account_status='Activate'";
			account_rs = stmt.executeQuery(sql);
			while(account_rs.next()) {
				Integer customer_id = account_rs.getInt("customer_id");
				Integer account_no = account_rs.getInt("account_no");
				BigDecimal salary = account_rs.getBigDecimal("balance");
				String branch = account_rs.getString("branch");
				AccountInfo accounts = new AccountInfo();
				accounts.setCustomer_id(customer_id);
				accounts.setAccount_no(account_no);
				accounts.setBalance(salary);
	...
	public ArrayList<CustomerInfo>  wholeCustomer_details() {
		ResultSet customer_rs = null;
		Statement stmt=null;
		ArrayList<CustomerInfo> customerList = new ArrayList();
		try {
			stmt = connection.createStatement();
			sql = "select * from customer_details where customer_status='Activate'";
			customer_rs = stmt.executeQuery(sql);
			while(customer_rs.next()) {
				Integer customer_id = customer_rs.getInt("customer_id");
				String name = customer_rs.getString("name");
				String city = customer_rs.getString("city");
